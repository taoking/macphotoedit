...
- [ ] Brush mask: on JPEG/HEIC and a user-authorised RAW-derived photo, paint overlapping hard and soft strokes, change size/feather/flow, erase, undo and clear; compare preview with a new-file export at full resolution. Repeat a large brush at the left, right, top, bottom and all four corners, including a stroke that crosses a crop boundary. Verify the visible stroke width/coverage is not shortened at an edge and remains aligned after window resize, crop/rotate, reopening the asset and switching between horizontal/vertical images.
- [ ] Local-mask transform geometry: on horizontal, vertical and square authorised photos, place a large radial mask near each visible left/right/top/bottom edge and all four corners, then compare its red overlay and rings with the actual rendered/exported effect. Repeat a radius crossing a crop boundary, `crop + 90° rotation`, `crop + flip`, and `crop + straighten + flip`; drag each visible handle and paint once, reopen the asset and export a new file. Confirm each adjustment affects the visually targeted source region, the radius is not shortened merely because its mathematical endpoint is outside the source/crop, and no geometry is interpreted as pre-transform screen coordinates.
